...
def test_log_activity_success(client, mocker):
    mocker.patch("app.blueprints.activities.routes._service.log_activity", return_value={"id": "act-1"})
...
def test_log_activity_error(client, mocker):
    ...
    mocker.patch("app.blueprints.activities.routes._service.log_activity", side_effect=AppError("Failed", 400))
    resp = client.post("/api/activities", json={
        "date": "2026-06-18",
        "transport": "car",
        "distance": 10.0,
...
def test_list_activities_success(client, mocker):
    mocker.patch("app.blueprints.activities.routes._service.list_activities", return_value=[{"id": "act-1"}])
    resp = client.get("/api/activities", headers=get_headers())
    assert resp.status_code == 200
    assert len(resp.get_json()["data"]) == 1

def test_get_activity_success(client, mocker):
    mocker.patch("app.blueprints.activities.routes._service.get_activity", return_value={"id": "act-1"})
    resp = client.get("/api/activities/act-1", headers=get_headers())
    assert resp.status_code == 200

def test_get_activity_not_found(client, mocker):
    mocker.patch("app.blueprints.activities.routes._service.get_activity", return_value=None)
    resp = client.get("/api/activities/act-1", headers=get_headers())
    assert resp.status_code == 404

def test_delete_activity_success(client, mocker):
    mocker.patch("app.blueprints.activities.routes._service.delete_activity", return_value=True)
