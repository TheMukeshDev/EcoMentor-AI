...
def test_get_recommendations_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_recommendations", return_value={"tips": ["tip1"]})
    resp = client.post("/api/ai/recommendations", json={
        "score": 50, "transport": "car", "food": "mixed", "ac_usage": "high", "plastic_waste": 1.0
    }, headers=get_headers())
    assert resp.status_code == 200
    assert resp.get_json()["data"]["tips"] == ["tip1"]

def test_get_weekly_report_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_weekly_report", return_value={"biggest_contributor": "car"})
...
def test_get_eco_personality_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_eco_personality", return_value={"personality": "Warrior"})
...
def test_get_daily_mission_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_daily_mission", return_value={"challenge": "Walk"})
...
def test_get_forecast_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_carbon_savings_forecast", return_value={"forecast_1_month_kg": 10.0})
    mocker.patch("app.blueprints.ai.routes._ai_service.get_recommendations", return_value={"tips": ["tip1"]})
...
def test_chat_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.chat", return_value={"response": "hello"})
...
def test_chat_history_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.get_conversation_history", return_value=[{"role": "user"}])
    resp = client.get("/api/ai/chat/history", headers=get_headers())
    assert resp.status_code == 200
    assert len(resp.get_json()["data"]) == 1

def test_chat_clear_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.clear_conversation")
    resp = client.post("/api/ai/chat/clear", headers=get_headers())
    assert resp.status_code == 200

def test_whats_if_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.whats_if", return_value={"carbon_saved": 5.0})
    resp = client.post("/api/ai/what-if", json={
        "transport": "car", "distance": 10.0, "food_type": "vegan", "ac_usage": "low", "plastic_waste": 1.0,
...
def test_feedback_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._ai_service.submit_feedback", return_value={"acknowledgment": "thanks"})
...
def test_coach_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._coach_service.get_coaching_plan", return_value={"plan": "Do this"})
    resp = client.get("/api/ai/coach", headers=get_headers())
    assert resp.status_code == 200
    
def test_sustainability_report_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._report_service.generate_report", return_value={"report": "Good job"})
    resp = client.get("/api/ai/sustainability-report", headers=get_headers())
    assert resp.status_code == 200

def test_habits_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._habit_service.get_habits", return_value=[{"habit": "Recycle"}])
    resp = client.get("/api/ai/habits", headers=get_headers())
    assert resp.status_code == 200

def test_carbon_forecast_success(client, mocker):
    mocker.patch("app.blueprints.ai.routes._forecast_service.get_forecast", return_value={"trend": "down"})
