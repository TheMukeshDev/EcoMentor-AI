...
def test_register_success(client, mocker):
    mocker.patch(
        "app.services.auth_service.AuthService.register_user", return_value={"id_token": "token123"}
    )
    resp = client.post(
        "/api/auth/register",
...
def test_register_error(client, mocker):
    ...
    mocker.patch(
        ...
        side_effect=AuthenticationError("Exists"),
    )
    resp = client.post(
        "/api/auth/register",
        json={"email": "test@example.com", "password": "password123", "name": "Test User"},
...
def test_login_success(client, mocker):
    mocker.patch(
        "app.services.auth_service.AuthService.login_user", return_value={"id_token": "token123"}
    )
    resp = client.post(
...
def test_get_profile_success(client, mocker):
    mocker.patch(
        "app.services.auth_service.AuthService.get_user_profile", return_value={"name": "Alice"}
    )
...
def test_get_profile_error(client, mocker):
    ...
    mocker.patch(
        "app.services.auth_service.AuthService.get_user_profile",
...
def test_update_profile_success(client, mocker):
    mocker.patch(
        "app.services.auth_service.AuthService.update_user_profile",
...
def test_google_auth_success(client, mocker):
    mocker.patch(
        "app.services.auth_service.AuthService.google_auth", return_value={"id_token": "token"}
    )
...
def test_google_auth_error(client, mocker):
    ...
    mocker.patch(
        "app.services.auth_service.AuthService.google_auth",
        side_effect=AuthenticationError("Invalid"),
    )
    resp = client.post("/api/auth/google", json={"id_token": "google123"})
    assert resp.status_code == 401
