...
def test_get_dashboard_success(client, mocker):
    mocker.patch(
        "app.services.dashboard_service.DashboardService.get_summary", return_value={"score": 100}
    )
...
def test_get_history_success(client, mocker):
    mocker.patch(
        "app.services.dashboard_service.DashboardService.get_history", return_value=[{"score": 100}]
    )
...
def test_get_history_error(client, mocker):
    ...
    mocker.patch(
        "app.services.dashboard_service.DashboardService.get_history",
        side_effect=AppError("Fail", 400),
    )
