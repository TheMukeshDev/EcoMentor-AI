...
class TestAuthRoutes:
    def test_register_post_returns_400_without_data(self, client):
        resp = client.post("/api/auth/register", content_type="application/json", data="{}")
    ...
    def test_profile_put_returns_401_without_auth(self, client):
        resp = client.put("/api/auth/profile", content_type="application/json", data="{}")
        assert resp.status_code == 401

    def test_google_auth_returns_400_without_token(self, client):
        resp = client.post("/api/auth/google", content_type="application/json", data="{}")
        assert resp.status_code in (400, 422)
...
class TestActivitiesRoutes:
    ...
    def test_create_activity_returns_401_without_auth(self, client):
        resp = client.post("/api/activities", content_type="application/json", data="{}")
        assert resp.status_code == 401
    ...
    def test_get_activity_returns_404_for_missing(self, client, app):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value={"uid": "test-user"}):
            with patch(
                "app.services.activity_service.ActivityService.get_activity",
                return_value=None,
            ):
                resp = client.get(
    ...
    def test_get_activity_owned_by_other_user_returns_403(self, client, app):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value={"uid": "test-user"}):
            with patch(
                "app.repositories.activity_repository.ActivityRepository.get",
...
class TestAIRoutes:
    def test_recommendations_returns_401_without_auth(self, client):
        resp = client.post("/api/ai/recommendations", content_type="application/json", data="{}")
        assert resp.status_code == 401
    ...
    def test_chat_stream_returns_401_without_auth(self, client):
        resp = client.post("/api/ai/chat/stream", content_type="application/json", data="{}")
        assert resp.status_code == 401
    ...
    def test_what_if_returns_401_without_auth(self, client):
        resp = client.post("/api/ai/what-if", content_type="application/json", data="{}")
        assert resp.status_code == 401

    def test_feedback_returns_401_without_auth(self, client):
        resp = client.post("/api/ai/feedback", content_type="application/json", data="{}")
        assert resp.status_code == 401
    ...
    def test_forecast_returns_success_with_auth(self, client):
        ...
        with patch("firebase_admin.auth.verify_id_token", return_value={"uid": "test-user"}):
            with (
                patch("app.services.ai_service.AIService.get_recommendations") as mock_rec,
                patch(
                ...
                resp = client.get(
                    "/api/ai/forecast", headers={"Authorization": "Bearer fake-token"}
                )
